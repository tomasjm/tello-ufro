...
def Send_Central_Stream(server_ip, token):
    ...
    ffmpeg.input("rtmp://192.168.8.160:1935/flash/11:admin:admin").output("rtmp://" + server_ip + ":1935/live/$camara1?token=" + token + "&movil=camion1", format='flv',vcodec='libx264').global_args("-timeout", "-1").run() # -timeout -1 (infinito)
...
def Send_Tello_Stream(server_ip, token):
    ...
    ffmpeg.input('udp:192.168.10.1:11111?overrun_nonfatal=1&fifo_size=50000000').output("rtmp://" + server_ip + ":1935/live/d1b4?token=" + token +"&movil=camion1", format='flv',vcodec='libx264').global_args("-timeout", "-1").run()
